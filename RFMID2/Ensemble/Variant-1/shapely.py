...
densenet = torch.load('models/strongdn_dn2.pth', map_location=torch.device('cpu'))
resnet = torch.load('models/strongdn_rn2.pth', map_location=torch.device('cpu'))
# densenet = torch.load('models/strongdn_dn-js.pth', map_location=torch.device('cpu'))
# resnet = torch.load('models/strongdn_rn-js.pth', map_location=torch.device('cpu'))
# densenet = torch.load('models/strongdn_dn-rfmid.pth', map_location=torch.device('cpu'))
# resnet = torch.load('models/strongdn_rn-rfmid.pth', map_location=torch.device('cpu'))
densenet.to(device)
resnet.to(device)


df = pd.read_csv('../../../../data/GT-main/set1/all.csv')
# image_names = ["2","18","aria_d_25_33","im0187","rf653"]
# image_names = ["493", "1054", "im0003","1326"]
# image_names = ["js793", "rf18", "js237", "js245", "im0007", "im0117"]
image_names = ["1271", "1386", "1689", "1550", "im0264", "rf283", "rf186"]
idx = df[df['ID'].isin(image_names)].index.tolist()
print(idx)

# filename1 = './shap/Drusen/all-model2.png'
# filename2 = './shap/Drusen/all-model2-info.txt'
# filename1 = './shap/DR/all-js.png'
# filename2 = './shap/DR/all-js-info.txt'
# filename1 = './shap/DR/all-rfmid.png'
# filename2 = './shap/DR/all-rfmid-info.txt'
filename1 = './shap/misc/all-model2-multilabel.png'
filename2 = './shap/misc/all-model2-info-multilabel.txt'
...
# threshold = [0.26, 0.21, 0.26, 0.41, 0.99, 0.5, 0.36, 0.26, 0.15, 0.68, 0.47, 0.41, 0.64, 0.4, 0.17, 0.48, 0.19, 0.13, 0.36, 0.55, 0.35]
# threshold = [0.4, 0.26, 0.31, 0.37, 0.49, 0.34, 0.36, 0.41, 0.38, 0.63, 0.58, 0.19, 0.35, 0.99, 0.5, 0.91, 0.54, 0.7, 0.74, 0.28, 0.1 ]
...
shap_values = explainer(images, max_evals=10000, batch_size=50, outputs=shap.Explanation.argsort.flip[:3])
torch.cuda.empty_cache()
