...
model = torch.load('models/dn201.pth', map_location=torch.device('cpu'))
model.to(device)


df = pd.read_csv('../../../data/GT-main/set1/all.csv')
# image_names = ["aria_a_13_2","im0151","js888","1592", "rb98", "rb27"]
# image_names = ["js301", "js290", "781.png","rf492","rf287", "237", "rf500", "im0073", "im0098","im0094","im0208"]
image_names = ["rb36","rb56","rb101","rb135"]
idx = []
for im in image_names: idx.extend(df[df['ID'] == im.split('.')[0]].index.tolist())
print(idx)

filename1 = './shap/new/dn201-2.png'
filename2 = './shap/new/dn201-info2.txt'
...
# threshold = [0.84, 0.21, 0.79, 0.11, 0.89, 0.9, 0.85, 0.05, 0.15, 0.81, 0.97, 0.06, 0.02, 0.89, 0.21, 0.95, 0.25, 0.23, 0.44, 0.04, 0.01]
threshold = [0.2, 0.33, 0.3, 0.41, 0.16, 0.01, 0.14, 0.31, 0.39, 0.03, 0.02, 0.16, 0.45, 0.03, 0.01, 0.01, 0.03, 0.92, 0.76, 0.79, 0.01]
